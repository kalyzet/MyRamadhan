...
_(M4 — skeleton flash & text field controller — sengaja ditunda karena touch UI besar.)_
...
## 7. Ringkasan Perubahan Fase 4 — Low Severity (L1–L6)

| #   | Fix                   | Detail                                                                                                                                                                                                                                                                          |
| --- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| L1  | Stub dihapus          | `recalculateStreaksFromDate` (stub kosong, tanpa referensi) dihapus dari `daily_record_repository.dart`; perhitungan streak kini sepenuhnya milik `StreakTrackerService.recalculateAllStreaks`.                                                                                 |
| L4  | Perfect day disatukan | `XpCalculatorService.isPerfectDay` menjadi statis publik = **single source of truth**. `StreakTrackerService.isPerfectDay` dan `AppState._isPerfectDay` mendelegasi ke sana — bonus XP, streak, dan UI tidak mungkin lagi berbeda definisi "hari sempurna".                     |
| L3  | RNG side quest        | Seed kini dari tanggal penuh (`day + month*31 + year*373`) dengan stride bervariasi per langkah. Tetap deterministik per tanggal (offline-safe), tapi tersebar merata di seluruh pool 10 quest — tidak lagi collapse ke segelintir triple; tahun berbeda memberi hasil berbeda. |
| L2  | FutureBuilder churn   | `StatsScreen` dikonversi ke StatefulWidget; future summary & records di-cache per `sessionId` sehingga rebuild akibat `notifyListeners` tidak lagi menembak query DB berulang / flash spinner.                                                                                  |
| L5  | createNewSession      | Panggilan `deactivateAllSessions()` yang redundan dihapus (`setActiveSession` sudah transaksional); init stats & achievement kini dilakukan SEBELUM aktivasi sehingga sesi aktif selalu punya data pendukung. Method `deactivateAllSessions` dipertahankan (dipakai test).      |
| L6  | DST edge              | Audit ulang semua `.difference().inDays`: seluruh sisa pemakaian sudah bekerja pada tanggal ternormalisasi midnight (validation service, repo, streak service) atau memakai `DateNormalizer.daysBetween`. Tidak ada aritmetika hari mentah tersisa.                             |

### Verifikasi Fase 4

- `flutter analyze`: tetap **0 error di `lib/`**.
- Test lulus individual: side_quest_repository (5), xp_calculator (15), streak_tracker (12), app_state (2), stats_screen (3), complete_workflows (6), offline_functionality (9).
- Catatan: menjalankan beberapa file test DB secara paralel kadang memicu `database is locked` (sqflite ffi shared file) — flakiness environment bawaan, bukan regresi; semua lulus saat dijalankan per file.
